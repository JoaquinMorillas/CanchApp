    ...
    const saveChanges = async () =>{
        try{
            ...
            if(files.length > 0){
                ...
                const response = await api.post("/images/upload", uploadForm)
    ...
    useEffect(() =>{
        const fetchSportsFields = async () =>{
            try{
                ...
                const response = await api.get(`/sport_field/stablishment/${id}`)
  ...
  return (
    ...
    {stablishment ? (
        ...
        style={{ backgroundColor: '#F8F9FA',  maxWidth: '700px', margin: '0 auto' }}>
            ...
            <div>
        {/* amenities inputs*/}
        <h5 className='text-center'>Caracteristicas</h5>
        <div className='text-center'>

            <button className={`btn ${enableSelectedAmenities ? "btn-primary" : "btn-danger"} mb-4`}
            onClick={() => setEnableSelectedAmenities(!enableSelectedAmenities)}>
                {enableSelectedAmenities ? "Editar" : "Confirmar"}
                </button>
        </div>
            ...
            {amenities?.length > 0 && 
              amenities.map((amenity, idx) => {
                ...
                <tr key={idx}>
                  ...
                  <td className='text-center'>
                    <input type="checkbox"
                    disabled={enableSelectedAmenities}
                    checked={isChecked}
                    onChange={handleChck} />
        ...
        style={{ backgroundColor: '#F8F9FA',  maxWidth: '700px', margin: '0 auto' }}>
            ...
            {sportFields && sportFields.length > 0 ? (
                <>
                            ...
                            <tr key={s.id}>
                                    ...
                                    <td>
                                        <div className="d-flex justify-content-center gap-2">
                                            <NavLink to={`/administracion/canchas/editar/${s.id}/crear_reservas`}>
                                                <button className='btn btn-warning'>Agregar Reservas</button>
                                            </NavLink>
                                            <NavLink to={`/administracion/canchas/editar/${s.id}`}>
                    ...
                    <div className="mb-3 d-flex justify-content-center">
                        <NavLink to={`/administracion/establecimientos/${id}/agregar_cancha`}>
